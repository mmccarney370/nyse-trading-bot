...
- **§ 3 Subsystems — Self-tuner (Apr-19 evening):** new `claude_tuner.py` provides an Anthropic-backed alternative to the Gemini 2.5 Flash tuner. Provider selected via `TUNER_PROVIDER ∈ {claude, gemini}` (default `claude` when `ANTHROPIC_API_KEY` is set, transparent fallback to Gemini on missing key / SDK / runtime error). Opus 4.7 uses ephemeral prompt caching on the static system prompt (mission + bounds + protocol + schema) and extended thinking (16k default budget). The prompt was rewritten from scratch to reward integrated strategic reasoning, cross-parameter interaction analysis, statistical-significance checks, and explicit rollback triggers, rather than the Gemini triage-tree formula. Both tuners share `save_dynamic_config`, `_PERSISTED_ABSOLUTE_BOUNDS`, and `log_structured_gemini_change` so the persistence and audit story is identical across providers.
