...
class OrderTracker:
    ...
    def __init__(self, filepath: str = ORDER_TRACKER_FILE):
        ...
        self._lock = threading.RLock()  # Thread safety: reentrant to prevent deadlock in stream.py _handle_replaced
        self._load()
    ...
    def mark_exit_fill(self, symbol: str, exit_order_id: str, fill_price: float) -> Optional[str]:
        ...
        with self._lock:
            ...
            if exit_order_id == group.trailing_stop_id:
                ...
                logger.warning(f"[TRACKER] {symbol} unknown exit order {exit_order_id} — state unchanged")
                return None
            self._save()
            return cancel_id
    ...
    def mark_closed(self, symbol: str):
        with self._lock:
            ...
            # Clean up index entries on close
            for oid in (group.entry_order_id, group.trailing_stop_id, group.take_profit_id):
                if oid:
